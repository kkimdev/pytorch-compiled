...
Tensor quantized_batch_norm(const Tensor & input, const Tensor & weight, const Tensor & bias, const Tensor & mean, const Tensor & var, double eps, double output_scale, int64_t output_zero_point) {
    if (input.has_names() || weight.has_names() || bias.has_names() || mean.has_names() || var.has_names()) {
        AT_ERROR(
            "quantized_batch_norm is not yet supported with named tensors. Please drop names via "
...
auto registerer = torch::RegisterOperators()
  ...
  .op(torch::RegisterOperators::options()
    ...
    .impl_unboxedOnlyKernel<Tensor (const Tensor &, const Tensor &, const Tensor &, const Tensor &, const Tensor &, double, double, int64_t), &QuantizedCPUType::quantized_batch_norm>(DispatchKey::QuantizedCPUTensorId)
    .aliasAnalysis(c10::AliasAnalysisKind::FROM_SCHEMA))
  .op(torch::RegisterOperators::options()
